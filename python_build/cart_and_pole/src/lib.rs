...
pub const CART_AND_POLE_NUM_DECISION_VARIABLES: usize = 25;

/// Number of parameters
pub const CART_AND_POLE_NUM_PARAMETERS: usize = 4;

/// Number of parameters associated with augmented Lagrangian
pub const CART_AND_POLE_N1: usize = 25;

/// Number of penalty constraints
pub const CART_AND_POLE_N2: usize = 0;
...
const CONSTRAINTS_BALL_RADIUS : f64 = 9.4;
...
const SET_C_XMIN :Option<&[f64]> = Some(&[-1.0,std::f64::NEG_INFINITY,-3.141592653589793,std::f64::NEG_INFINITY,]);
const SET_C_XMAX :Option<&[f64]> = Some(&[1.0,std::f64::INFINITY,3.141592653589793,std::f64::INFINITY,]);
...
/// Y_min
const SET_Y_XMIN :Option<&[f64]> = Some(&[-1000000000000.0, 0.0, -1000000000000.0, 0.0]);

/// Y_max
const SET_Y_XMAX :Option<&[f64]> = Some(&[1000000000000.0, 0.0, 1000000000000.0, 0.0]);
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(CART_AND_POLE_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, CART_AND_POLE_N1, CART_AND_POLE_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_cart_and_pole::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), CART_AND_POLE_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), CART_AND_POLE_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_cart_and_pole::init_cart_and_pole();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_cart_and_pole::precondition(u, p);

        // Compute initial penalty
        icasadi_cart_and_pole::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_cart_and_pole::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_cart_and_pole::grad(u, xi, p, grad);
        Ok(())
    };
    
    let f1 = |u: &[f64], res: &mut [f64]| -> Result<(), SolverError> {
        icasadi_cart_and_pole::mapping_f1(u, p, res);
        Ok(())
    };
