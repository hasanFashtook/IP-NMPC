...
pub const BALL_AND_PLATE_NUM_DECISION_VARIABLES: usize = 15;

/// Number of parameters
pub const BALL_AND_PLATE_NUM_PARAMETERS: usize = 4;

/// Number of parameters associated with augmented Lagrangian
pub const BALL_AND_PLATE_N1: usize = 0;

/// Number of penalty constraints
pub const BALL_AND_PLATE_N2: usize = 0;
...
pub fn initialize_solver() -> AlmCache {
    let panoc_cache = PANOCCache::new(BALL_AND_PLATE_NUM_DECISION_VARIABLES, EPSILON_TOLERANCE, LBFGS_MEMORY);
    AlmCache::new(panoc_cache, BALL_AND_PLATE_N1, BALL_AND_PLATE_N2)
}
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_ball_and_plate::get_w_cost();
...
pub fn solve(
    ...
    assert_eq!(p.len(), BALL_AND_PLATE_NUM_PARAMETERS, "Wrong number of parameters (p)");
    assert_eq!(u.len(), BALL_AND_PLATE_NUM_DECISION_VARIABLES, "Wrong number of decision variables (u)");

    // Start by initialising the optimiser interface (e.g., set w=1)
    icasadi_ball_and_plate::init_ball_and_plate();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_ball_and_plate::precondition(u, p);

        // Compute initial penalty
        icasadi_ball_and_plate::initial_penalty(u, p, & mut rho_init);
    }

    let psi = |u: &[f64], xi: &[f64], cost: &mut f64| -> Result<(), SolverError> {
        icasadi_ball_and_plate::cost(u, xi, p, cost);
        Ok(())
    };
    let grad_psi = |u: &[f64], xi: &[f64], grad: &mut [f64]| -> Result<(), SolverError> {
        icasadi_ball_and_plate::grad(u, xi, p, grad);
        Ok(())
    };
